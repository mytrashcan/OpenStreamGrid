...
OpenStreamGrid는 MPEG-TS HLS 스트리밍 서비스에 피어 기반
세그먼트 전송을 추가합니다. 같은 라이브 방송을 시청하는 클라이언트가 이미
받은 영상 세그먼트를 서로 공유하므로 Origin 서버와 CDN의 전송량을 줄일 수
있습니다.

LL-HLS와 CMAF/fMP4는 로드맵 항목이며 이 릴리스에서는 지원하지 않습니다.
...
| 변수 | 기본값 | 설명 |
| --- | --- | --- |
| `PORT` | `7070` | HTTP 수신 포트 |
| `HOST` | `0.0.0.0` | 수신 주소 |
| `STALE_PEER_MS` | `30000` | 비활성 피어 만료 시간(ms) |
| `STORE_TYPE` | `sqlite` | 저장소 유형: `sqlite` 또는 `memory` |
| `DB_PATH` | `./data/tracker.db` | SQLite 데이터베이스 경로 |
| `TRACKER_API_KEY` | 미설정 | 방송 관리·모니터링용 관리자 키 |
| `PEER_SESSION_SECRET` | 프로세스별 임의 값 | 32바이트 이상의 피어 세션 서명 키 (배포 시 명시 필수) |
| `PEER_SESSION_TTL_MS` | `3600000` | 피어 세션 수명(ms) |
...
| --- | --- | --- |
| `TRACKER_URL` | `http://tracker:7070` | Tracker 기본 URL |
| `TRACKER_API_KEY` | 미설정 | 선택적 관리자 키. 일반 피어 트래픽은 발급된 세션 토큰 사용 |
...
| `hashUrlResolver` | 인접 `.sha256` | 서명·쿼리 URL을 고려한 해시 파일 주소 구성 함수 |
...
### Phase 3 스케줄러 정책 실험: `mixed-workload`

| 정책 | 데드라인 충족 | Peer 승리 | Origin 승리 | 헤지 시작 | 완료 p50 | 완료 p95 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Node-legacy | 85% | 77.5% | 22.5% | 0% | 860.54 ms | 1,365.97 ms |
| Node-deadline-aware | 100% | 63.75% | 36.25% | 6.75% | 769.12 ms | 1,332.58 ms |
| Browser-legacy | 84.5% | 98.75% | 1.25% | 0% | 801.72 ms | 1,271.02 ms |
| Browser-deadline-aware | 100% | 81.5% | 18.5% | 0.75% | 722.18 ms | 1,265.83 ms |

_고정 시드 `phase3-mixed-2026`, 정책별 결정론적 요청 400건. 프로덕션 스케줄러를 사용하는 통제된 시뮬레이션이며 프로덕션 수용량 주장이 아닙니다. [스케줄러 실험실](benchmarks/scheduler-lab/README.md)을 참고하세요._
<!-- BENCHMARK_SCHEDULER:END -->

<!-- BENCHMARK:START -->
_Benchmark data is generated by running `bash scripts/benchmark.sh --scenario benchmarks/scenarios/ci-smoke.json`. See [benchmark methodology](docs/benchmark-methodology.md) for details._
<!-- BENCHMARK:END -->
